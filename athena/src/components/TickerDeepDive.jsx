...
function DeepDiveContent({ data }) {
  ...
  const bs = data?.balanceSheet || {}; // legacy fallback, kept for backwards-compat
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ScorePanel title="Valuation" score={v.score} weight="40%" color="text-cyan-300" icon={DollarSign}>
          ...
          <KV k="P/S" v={v.priceSales} />
          <KV k="P/FCF" v={v.priceFcf} />
          <KV k="EV/EBITDA" v={v.evToEbitda} />
          <KV k="Earnings Yield" v={v.earningsYield} />
          <KV k="Dividend Yield" v={v.dividendYield} />
          <KV k="vs Sector Median" v={v.vsSectorMedianPe} />
        </ScorePanel>

        <ScorePanel title="Growth & Quality" score={g.score} weight="30%" color="text-emerald-300" icon={TrendingUp}>
          ...
          <KV k="FCF Margin" v={g.fcfMargin} />
          <KV k="ROE" v={g.roe} />
          <KV k="ROIC" v={g.roic} />
          <KV k="FCF Growth" v={g.fcfGrowth} />
          <KV k="Piotroski F-Score" v={g.piotroskiScore} />
        </ScorePanel>
      ...
      {/* Market sentiment (forward-looking) */}
      {data?.marketSentiment && (
        <MarketSentimentPanel sentiment={data.marketSentiment} />
      )}

      {/* Financial strength */}
      {data?.financialStrength && (
        <FinancialStrengthPanel strength={data.financialStrength} />
      )}
