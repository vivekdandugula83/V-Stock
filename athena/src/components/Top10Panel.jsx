...
export default function Top10Panel({ picks = [], onTickerClick, watchlist = [], onToggleWatch, limit = 15 }) {
    ...
    <div className="space-y-3">
      ...
      <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="display text-lg text-white">Top {safe.length} Picks{filters.length > 0 ? ' (filtered)' : ''}</h2>
      ...
      <div className="divide-y divide-white/5">
        {safe.length === 0 && (
          <div className="px-6 py-8 text-center text-stone-500 text-sm">
            No picks match these filters. <button onClick={() => { setFilters([]); setSearch(''); }} className="underline hover:text-amber-300">Clear filters</button>
          </div>
        )}
        {safe.map((p, i) => {
